...
CREATE TEMP TABLE research_agriguard_covers_measurements (
...
BEGIN
  ...
  LOOP
    longitude := CASE target_count WHEN 100 THEN 107.0504 ELSE 107.5004 END;
    ...
    LOOP
      ...
      FROM research_agriguard_covers_geometries
      WHERE id <= target_count
        AND ST_Covers(polygon, ST_SetSRID(ST_Point(longitude, latitude), 4326))
      LIMIT 1;
      IF iteration > 3 THEN
        INSERT INTO research_agriguard_covers_measurements
        VALUES (
          target_count,
          iteration - 3,
...
\copy (SELECT polygon_count, run, execution_ms, found FROM research_agriguard_covers_measurements ORDER BY polygon_count, run) TO STDOUT WITH CSV HEADER
